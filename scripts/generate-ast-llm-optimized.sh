...
echo "=== Generating AST for LLM Consumption (Memory-Optimized) ==="
echo ""

# Create output directory if it doesn't exist
mkdir -p ast-output-llm

echo "Starting AST analysis with increased memory allocation..."
echo "This may take a few minutes depending on the size of the codebase."

# Run the TypeScript script using ts-node with increased memory allocation
NODE_OPTIONS="--max-old-space-size=4096" npx ts-node scripts/ast-for-llm-simple.ts
...
if [ $? -eq 0 ]; then
  ...
  echo ""
  echo "If you're experiencing memory issues, try the following:"
  echo "1. Close other memory-intensive applications"
  echo "2. Increase the memory allocation by editing this script and changing --max-old-space-size=4096 to a higher value"
  echo "3. Run the script on a machine with more memory"
  exit 1
fi
